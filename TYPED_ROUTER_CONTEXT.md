...
4.  **`.lazy()` method**: The `RouteBuilder` instance has a `.lazy()` method for defining lazy-loaded properties. This method takes a function that returns a `Promise` for an object containing the `component` and an optional `resolve` object.
