...
`onSameUrlNavigation` controls how the Router handles navigations to the current URL. The default `'ignore'` skips processing, while `'reload'` runs the URL through the navigation pipeline. Guards and resolvers rerun according to `runGuardsAndResolvers`, and component reuse is governed by `RouteReuseStrategy`.
