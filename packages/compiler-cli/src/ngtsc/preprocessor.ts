...
import {
  CompilationTicket,
  CompilationTicketKind,
  freshCompilationTicket,
  incrementalFromCompilerTicket,
} from './core/src/compiler';
import {ivyTransformFactory} from './transform';
 ...
 *
 * This mode is designed for the "Connected" architecture where the Angular Compiler
 * acts as a pre-processor for a downstream TypeScript compiler (e.g. TSGo).
 */
export class NgtscIsolatedPreprocessor {
  ...
  constructor(
    ...
    if (oldProgram === undefined) {
      ticket = freshCompilationTicket(
        ...
        programDriver,
        perfRecorder,
        /* enableTemplateTypeChecker */ true, // Always enable for TCB generation
  ...
  transformAndPrint(): {fileName: string; content: string}[] {
    // 1. Generate TCBs
    this.compiler.getTemplateTypeChecker().generateAllTypeCheckBlocks();
    ...
    // TODO: Add separate Type Reification transformer here if not covered by ivyTransformFactory
