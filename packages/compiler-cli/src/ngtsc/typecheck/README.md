...
The output must remain valid TypeScript source text using `import()` types for external references. `DetachedTcbEnvironment.referenceType` has been successfully migrated to emit inline `import('...').MyComponent` types (via `ts.factory.createImportTypeNode`), eliminating the need for `ts.factory.createQualifiedName` and simplifying the generated virtual AST payload by cleanly avoiding top-level prelude statements.
