...
## Pre-processor TCB Generation (Source-to-Source)

The above architecture assumes the TCB generator is running within a live `ts.Program` containing a full `ts.TypeChecker`, `ReflectionHost`, and a tightly-coupled dependency graph. In a decoupled, source-to-source pre-processor architecture (e.g., driven by a native TS Go or Rust analyzer), the TCB generation phase must be entirely detached from this stateful context.

This requires shifting the TCB generator from a stateful, symbol-based system to a completely detached, standalone metadata-driven generator.

### 1. Decoupling from `ts.Program` and `ts.TypeChecker`

The most significant change involves breaking the dependency on TypeScript's semantic APIs (`ts.TypeChecker` and `ts.Symbol`) during code generation.

- **Why not just create a smaller `ts.Program` from the pre-processor's `.d.ts` output?** While a native TS Go implementation is exceptionally fast, creating an intermediate `ts.Program` strictly to execute the TCB generator adds an unnecessary parse-and-bind cycle. The native pre-processor has already extracted inputs, outputs, signal statuses, and coercion types. Feeding this structured metadata directly into the JS TCB generator bypasses the need to instantiate, bind, and query a `ts.Program`, vastly reducing memory and CPU utilization.

### 2. Detached Metadata Interfaces

- **String Snippets & Type Node Parsing:** Since the Rust pre-processor communicates pure string structures over IPC (e.g., `"<T>"` or `"boolean | string"`), the JS compiler must parse these strings back into valid `ts.TypeNode` instances to be used by the AST emitter. Utilities for this deserialization (e.g., `parseTypeStringAsNode`) are provided in `api/tcb_util.ts` and must **only** be invoked outside the pure TCB generator block (e.g., inside the adapter or deserialization layer).

A new abstraction layer (`api/tcb_metadata.ts`) strictly defines the serializable boundaries between the analyzer frontend and the JS backend compiler:

- **`TcbDirectiveMetadata` & `TcbComponentMetadata`:** These interfaces replace `DirectiveMeta`. They define the shape, selector, inputs, and outputs of components using pure data structures (e.g., `ts.TypeNode` snippets) rather than `Reference` objects attached to a live `ts.SourceFile`.
- **Explicit Signal and Transform State:** Inputs are represented as flat, structured arrays (`tcbInputs: Iterable<...>`) containing flags like `isSignal` and raw AST snippets for type extraction like `transform.type.node: ts.TypeNode`. The type nodes are generated by the native analyzer and injected directly into the AST, completely circumventing decorators lookup.

### 3. Simplified Import Management

In the classical architecture, `ModuleResolver` dynamically resolved the exact relative paths or module specifiers for importing dependencies during TCB generation.

- **Pre-processor Design:** The analyzer explicit records the `moduleName` or `bestGuessOwningModule.specifier` directly on every `TcbDirectiveMetadata` and `TcbPipeMetadata` object.
- **`ImportManager` Adaptation:** The TCB generator uses a minimal lookup mechanism to build `import ... from '...'` statements purely based on these provided string specifiers, side-stepping the compilation graph entirely.

### 4. Determinism and Cacheability Challenges

Because it evaluates pure metadata structures, the detached TCB generator operates deterministically, allowing for highly efficient caching.

- **The Over-Invalidation Problem:** The generator digests serialized `TcbDirectiveMetadata`. If we naively hash the metadata for _all_ directives in scope, a harmless additive change (e.g., appending a new `@Input()` to a common directive) will change the scope hash. This would instantly invalidate the cached TCB for _every_ component that incorporates that directive, even if those components don't bind to the new input.
- **Path Forward:** Optimal cacheability requires a fine-grained tracking mechanism that only hashes the specific inputs, outputs, and generic bounds _actually referenced_ by the component's template. The shift to a pure function abstraction provides the foundation for explicit cache dependency tracking.

### 5. Scope Compilation and Separation of Concerns

The native pre-processor and the JavaScript-based compiler divide labor to construct `NgModule` scopes and calculate `BoundTarget` elements.
...
Currently, `adaptTypeCheckBlockMetadata` (inside `tcb_adapter.ts`) conforms to this optimized separation by explicitly iterating over only the directives in the `BoundTarget`:
...
This guarantees the TCB generator receives a minimal, hyper-optimized payload (`TcbDirectiveMetadata`) encompassing exclusively the directives / pipes actively utilized by the template, filtering out the "entire world" of the NgModule.

**Future Native Pre-Processor Implementation Steps (Rust/TS Go)**
Moving to the true native pre-processor requires integration changes in the JS compiler's front door to implement a **registry / dictionary of IDs** pattern to prevent redundant scope metadata payload limits over the IPC bridge:

1. **Assign Unique IDs (Pre-processor):** The native pre-processor will generate a unique ID (e.g., `[filePath]#[className]`) for every analyzed directive and pipe.
2. **Global Dictionary Payload (Pre-processor):** Across the IPC bridge, the pre-processor will serialize a global `dependencies` dictionary mapping these string IDs to their full serialized `TcbDirectiveMetadata` and `TcbPipeMetadata` properties.
