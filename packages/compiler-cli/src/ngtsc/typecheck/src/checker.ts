...
export class TemplateTypeCheckerImpl implements TemplateTypeChecker {
  ...
  private getSymbolsForBoundEvent(eventBinding: TmplAstBoundEvent, tcb: ts.Node) {
    ...
    const node = findNodeWithSourceSpan(eventBinding.sourceSpan, tcb, ts.isCallExpression);
    if (node === null) {
      return [];
    }
  ...
  private getSymbolsFromInputBinding(attributeBinding: TmplAstBoundAttribute, tcb: ts.Node) {
    const node = findNodeWithSourceSpan(attributeBinding.sourceSpan, tcb, ts.isBinaryExpression);
    if (node === null) {
      return [];
    }
    const typeChecker = this.typeCheckingStrategy.getProgram().getTypeChecker();
