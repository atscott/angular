...
export abstract class TcbDirectiveTypeOpBase extends TcbOp {
  ...
  override execute(): ts.Identifier {
    const rawType = this.tcb.env.referenceType(this.dir);

    let type: ts.TypeNode;
    let span: ParseSourceSpan;
    if (this.dir.isGeneric === false || this.dir.typeParameters === undefined) {
      type = rawType;
    } else {
      if (!ts.isTypeReferenceNode(rawType)) {
        throw new Error(
          `Expected TypeReferenceNode when referencing the type for ${this.dir.name}`,
        );
      }
      const typeArguments = this.dir.typeParameters.map(() =>
...
export class TcbNonGenericDirectiveTypeOp extends TcbDirectiveTypeOpBase {
  ...
  override execute(): ts.Identifier {
    if (this.dir.isGeneric) {
      throw new Error(`Assertion Error: expected ${this.dir.name} not to be generic.`);
    }
...
export class TcbGenericDirectiveTypeWithAnyParamsOp extends TcbDirectiveTypeOpBase {
  override execute(): ts.Identifier {
    if (this.dir.typeParameters === undefined) {
      throw new Error(
        `Assertion Error: expected typeParameters when creating a declaration for ${this.dir.name}`,
      );
    }
