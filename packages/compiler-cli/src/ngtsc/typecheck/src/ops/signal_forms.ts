...
export function isFieldDirective(meta: TcbDirectiveMetadata): boolean {
  ...
  return moduleName !== undefined && moduleName.includes('@angular/forms');
}
...
function getSyntheticFieldBoundInput(
  ...
  const inputs = dir.tcbInputs.filter(
    (i: import('../../api/tcb_metadata').TcbInputMapping) => i.bindingPropertyName === inputName,
  );
  ...
  return {
    ...
    inputs: inputs.map((input: import('../../api/tcb_metadata').TcbInputMapping) => {
      // If we're setting up the `value` binding of a custom component, we assign
      // it to the same field, but we type it to the directive's specific type.
...
function isControlValueAccessorLike(meta: TcbDirectiveMetadata): boolean {
  return meta.hasControlValueAccessorMethods;
}
