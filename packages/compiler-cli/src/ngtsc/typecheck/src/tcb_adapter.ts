...
function adaptDirective(dir: TypeCheckableDirectiveMeta, env: Environment): TcbDirectiveMetadata {
  ...
  const tcbDir: TcbDirectiveMetadata = {
    ...
    typeParameters: (dir.ref.node as unknown as ts.ClassDeclaration).typeParameters
      ? Array.from((dir.ref.node as unknown as ts.ClassDeclaration).typeParameters!).map((p) =>
          ts.factory.createTypeParameterDeclaration(
            undefined,
            ts.factory.createIdentifier(p.name.text),
            undefined,
  ...
  Object.defineProperty(tcbDir, 'ref', {value: dir.ref, enumerable: false, writable: true});
  return tcbDir;
}
