...
import {addParseSpanInfo, addTemplateId, CommentTriviaType, ExpressionIdentifiers, ignoreDiagnostics, wrapForDiagnostics} from './diagnostics';
...
class TcbDirectiveTypeOp extends TcbOp {
  ...
  execute(): ts.Identifier {
    ...
    ts.addSyntheticTrailingComment(
        type, ts.SyntaxKind.MultiLineCommentTrivia,
        `${CommentTriviaType.EXPRESSION_TYPE_IDENTIFIER}:${ExpressionIdentifiers.DIRECTIVE}`,
...
class TcbReferenceOp extends TcbOp {
  constructor(
      private tcb: Context, private scope: Scope, private node: TmplAstReference,
      private host: TmplAstElement|TmplAstTemplate,
      private target: TypeCheckableDirectiveMeta|TmplAstTemplate|TmplAstElement) {
    super();
  }
...
export class TcbDirectiveOutputsOp extends TcbOp {
  ...
  static decodeOutputCallExpression(node: ts.CallExpression): ts.ElementAccessExpression|null {
    ...
    if (outputFieldAccess === undefined || !ts.isElementAccessExpression(outputFieldAccess)) {
      return null;
    }
...
class Scope {
  ...
  render(): ts.Statement[] {
    for (let i = 0; i < this.opQueue.length; i++) {
      ...
      if (this.tcb.env.config.enableTemplateTypeChecker) {
        this.executeOp(i, /* skipOptional */ false);
      } else {
