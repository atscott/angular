  ...
  describe('TemplateTypeChecker.getSymbolOfNodeInComponentTemplate', () => {
    ...
    describe('templates', () => {
      describe('ng-templates', () => {
        ...
        it('should get symbol for variables at the declaration', () => {
          const symbol = templateTypeChecker.getSymbolOfNodeInComponentTemplate(
              templateNode.variables[0], cmp)!;
        ...
        it('should get symbol for the template itself', () => {
          const symbol = templateTypeChecker.getSymbolOfNodeInComponentTemplate(templateNode, cmp)!;
      ...
      describe('structural directives', () => {
        ...
        it('should retrieve a symbol for property reads of implicit variable inside structural binding',
           () => {
             ...
             const userSymbol = templateTypeChecker.getSymbolOfNodeInComponentTemplate(
                 namePropRead.receiver, cmp)!;
             expectUserSymbol(userSymbol);
           });

        it('finds symbols for variables', () => {
          const userVar = templateNode.variables.find(v => v.name === 'user')!;
          const userSymbol = templateTypeChecker.getSymbolOfNodeInComponentTemplate(userVar, cmp)!;
          expectUserSymbol(userSymbol);

          const iVar = templateNode.variables.find(v => v.name === 'i')!;
          const iSymbol = templateTypeChecker.getSymbolOfNodeInComponentTemplate(iVar, cmp)!;
        ...
        it('finds symbol when using a template variable', () => {
          ...
          const indexSymbol = templateTypeChecker.getSymbolOfNodeInComponentTemplate(
    ...
    describe('for expressions', () => {
      ...
      it('should get symbol for local reference of an Element', () => {
        ...
        const refSymbol =
            templateTypeChecker.getSymbolOfNodeInComponentTemplate(nodes[0].references[0], cmp)!;
        ...
        const myRefUsage =
            templateTypeChecker.getSymbolOfNodeInComponentTemplate(nodes[1].inputs[0].value, cmp)!;
      ...
      it('should get symbols for references which refer to directives', () => {
        ...
        const ref2Declaration =
            templateTypeChecker.getSymbolOfNodeInComponentTemplate(nodes[1].references[0], cmp)!;
        ...
        const dirValueSymbol =
            templateTypeChecker.getSymbolOfNodeInComponentTemplate(nodes[2].inputs[0].value, cmp)!;
        assertExpressionSymbol(dirValueSymbol);
        expect(dirValueSymbol.tsSymbol!.escapedName.toString()).toBe('dirValue');
        expect(program.getTypeChecker().typeToString(dirValueSymbol.tsType)).toEqual('string');

        const dir1Symbol =
            templateTypeChecker.getSymbolOfNodeInComponentTemplate(nodes[2].inputs[1].value, cmp)!;
        ...
        const dir2Symbol =
            templateTypeChecker.getSymbolOfNodeInComponentTemplate(nodes[3].inputs[1].value, cmp)!;
    ...
    describe('input bindings', () => {
      it('can retrieve a symbol for an input binding', () => {
        ...
        const aSymbol = templateTypeChecker.getSymbolOfNodeInComponentTemplate(inputAbinding, cmp)!;
        assertInputBindingSymbol(aSymbol);
        expect((aSymbol.tsSymbol!.declarations[0] as ts.PropertyDeclaration).name.getText())
            .toEqual('inputA');

        const inputBbinding = (nodes[0] as TmplAstElement).inputs[1];
        const bSymbol = templateTypeChecker.getSymbolOfNodeInComponentTemplate(inputBbinding, cmp)!;
        assertInputBindingSymbol(bSymbol);
        expect((bSymbol.tsSymbol!.declarations[0] as ts.PropertyDeclaration).name.getText())
            .toEqual('inputB');
      });
      ...
      it('can retrieve a symbol for an input of structural directive', () => {
        ...
        const symbol = templateTypeChecker.getSymbolOfNodeInComponentTemplate(ngForOfBinding, cmp)!;
        assertInputBindingSymbol(symbol);
        expect((symbol.tsSymbol!.declarations[0] as ts.PropertyDeclaration).name.getText())
            .toEqual('ngForOf');
      });
      ...
      it('returns empty list when there is no directive registered for the binding', () => {
        ...
        const symbol = templateTypeChecker.getSymbolOfNodeInComponentTemplate(binding, cmp);
        expect(symbol).toBeNull();
      });
      ...
      it('returns empty list when directive members do not match the input', () => {
        ...
        const inputAbinding = (nodes[0] as TmplAstElement).inputs[0];
        const symbol = templateTypeChecker.getSymbolOfNodeInComponentTemplate(inputAbinding, cmp);
        expect(symbol).toBeNull();
      });
      ...
      it('can match binding when there are two directives', () => {
        ...
        expect((symbol.tsSymbol!.declarations[0] as ts.PropertyDeclaration).name.getText())
            .toEqual('inputA');
        expect((symbol.tsSymbol!.declarations[0] as ts.PropertyDeclaration).parent.name?.text)
            .toEqual('TestDir');
      });
    ...
    describe('output bindings', () => {
      it('should find symbol for output binding', () => {
        ...
        const aSymbol =
            templateTypeChecker.getSymbolOfNodeInComponentTemplate(outputABinding, cmp)!;
        ...
        const bSymbol =
            templateTypeChecker.getSymbolOfNodeInComponentTemplate(outputBBinding, cmp)!;
      ...
      it('should find symbol for output binding when there are multiple directives', () => {
        ...
        const outputABinding = (nodes[0] as TmplAstElement).outputs[0];
        const symbol = templateTypeChecker.getSymbolOfNodeInComponentTemplate(outputABinding, cmp)!;
