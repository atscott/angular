...
function dispatchNavigateEvent({
  ...
  event.intercept = function (
    ...
    if (event.interceptionState !== 'none' && event.interceptionState !== 'intercepted') {
      throw new Error(
        'Event interceptionState should be "none" or "intercepted" when intercept() is called.',
      );
    }
