...
const guardPropertyNames = new Set([
  'canActivate',
  'canMatch',
  'canDeactivate',
  'canActivateChild',
  ...
  for (const migratableRoute of usages) {
    ...
    if (guards) {
      for (const property of guards) {
        ...
        if (
          (classGuards.length === 0 && injectionTokenGuards.length === 0) ||
        ...
        const mappedClassGuards = ts.factory.createCallExpression(
          ts.factory.createIdentifier(mapperFunctionName),
          undefined,
        ...
        if (nonClassGuards.length === 0 && injectionTokenGuards.length === 0) {
          ...
          const replacementNode = ts.factory.updatePropertyAssignment(
            property,
            property.name,
            mappedClassGuards,
          );
...
function getMigratableRoutes(sourceFile: ts.SourceFile): MigratableRoute[] {
  ...
  const visitNode = (node: ts.Node) => {
    if (ts.isObjectLiteralExpression(node)) {
      const hasRequiredRouteProperty = node.properties.some((prop) =>
        isPropertyAssignmentWithName(prop, requiredRouteProperties),
      );
      if (hasRequiredRouteProperty) {
        const route: MigratableRoute = {};

        route.guards = node.properties.filter((prop): prop is ts.PropertyAssignment =>
          isPropertyAssignmentWithName(prop, guardPropertyNames),
        );

        route.title = node.properties.find((prop): prop is ts.PropertyAssignment =>
          isPropertyAssignmentWithName(prop, 'title'),
        );

        const resolve = node.properties.find((prop): prop is ts.PropertyAssignment =>
          isPropertyAssignmentWithName(prop, 'resolve'),
        );
...
function isPropertyAssignmentWithName(
  ...
  nameOrNames: string | Set<string>,
): property is ts.PropertyAssignment {
  return (
    ts.isPropertyAssignment(property) &&
    ts.isIdentifier(property.name) &&
    (nameOrNames instanceof Set
      ? nameOrNames.has(property.name.text)
