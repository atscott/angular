...
import {
  ...
  FunctionalGuardsMigration,
} from '../../migrations/functional-guards/functional-guards-migration';
...
export function migrate(options: Options): Rule {
  return async (tree, context) => {
    ...
    const migration = new FunctionalGuardsMigration({
      shouldMigrate: (file) => {
        return (
    ...
    const unitResults: FunctionalGuardsCompilationUnitData[] = [];
    ...
    context.logger.info('');
    context.logger.info(`Successfully migrated to functional guards 🎉`);
    context.logger.info(
      `  -> Migrated ${replacementCount} components to functional guards in ${touchedFilesCount} component files.`,
    );
  };
}
