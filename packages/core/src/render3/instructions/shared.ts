...
import {getComponentLViewByIndex, getNativeByIndex, getNativeByTNode, getTNode, isCreationMode, isViewAttached, readPatchedLView, resetPreOrderHookFlags, unwrapLView, updateTransplantedViewCount} from '../util/view_utils';
...
export function refreshView<T>(
  ...
  try {
    ...
    // `LView` but is declared appears after the insertion component.
 ...
 *
 * @param lView The `LView` may have transplanted views.
 */
...
function refreshContainsDirtyView(lView: LView) {
  ...
  if (components !== null) {
    for (let i = 0; i < components.length; i++) {
      ...
      // Only attached components that are CheckAlways or OnPush and dirty should be refreshed
      if (isViewAttached(componentView) && componentView[TRANSPLANTED_VIEWS_TO_REFRESH] > 0) {
        refreshContainsDirtyView(componentView);
      }
    }
  }
