...
import {ChangeDetectionStrategy, ChangeDetectorRef, Component, Input, signal, untracked, ViewChild, WritableSignal} from '@angular/core';
import {TestBed} from '@angular/core/testing';

describe('change detection for signal views', () => {
  it('can create a signal component with a static binding', () => {
    @Component({
    ...
    expect(trim(fixture.nativeElement.textContent)).toEqual('hello world');
  });
  ...
  xit('re-dirties parent after initial change detection (No more ExpressionChangedAfterChecked for signal views)',
      () => {
        @Component({
  ...
  xit('doesn\'t change detect infinitely', () => {
    @Component({
      selector: 'child-component',
      standalone: true,
    ...
    class ChildComponentA {
      @Input() signalFromParent!: WritableSignal<number>;

      updateParentSignal() {
        untracked(() => this.signalFromParent.update(v => ++v));
      }
    }
    @Component({
      selector: 'parent-component',
    ...
    expect(() => {
      fixture.detectChanges();
    }).toThrowError(/Infinite change detection/);
  });

  xit('can re-enter change detection if a sibling view needs another refresh', () => {
    @Component({
      selector: 'parent-component',
      signals: true,
      standalone: true,
