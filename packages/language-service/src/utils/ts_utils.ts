...
import {SymbolReference} from '@angular/compiler-cli/src/ngtsc/typecheck/api';
...
export function getCodeActionToImportTheDirectiveDeclaration(
  ...
  if (node && node.name) {
    ref = new Reference(node as unknown as ClassDeclaration);
    if (directive.ref.moduleSpecifier) {
      (ref as any).bestGuessOwningModule = {
        specifier: directive.ref.moduleSpecifier,
        moduleName: directive.ref.moduleSpecifier,
      };
    }
  }
