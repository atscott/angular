...
class ApplyRedirects {
  ...
  private expandChildren(
    ...
    // expand outlets one at a time, starting with the primary outlet
    return from(Object.keys(segmentGroup.children).sort((a, b) => a === 'primary' ? -1 : 1))
        .pipe(
            concatMap(childOutlet => {
              const child = segmentGroup.children[childOutlet];
              const routesWithMatchingOutlet = routes.filter(r => getOutlet(r) === childOutlet);
              return this.expandSegmentGroup(ngModule, routesWithMatchingOutlet, child, childOutlet)
                  .pipe(
                      catchError((e) => {
                        if (e instanceof NoMatch) {
                          // If that fails, try matching against routes for other outlets. This
                          // would include empty path matches to different outlets.
                          const routesForOtherOutlets =
                              routes.filter(r => getOutlet(r) !== childOutlet);
                          return this.expandSegmentGroup(
                              ngModule, routesForOtherOutlets, child, childOutlet);
                        }
                        return throwError(e);
                      }),
                      map(s => ({segment: s, outlet: childOutlet})),
                  );
            }),
            scan(
