...
import {RouterResourcesFeatureImplementation} from '../router_resource_feature';
import {Data} from '../models';
import {ChildrenOutletContexts} from '../router_outlet_context';
import {ActivatedRoute} from '../router_state';
import {PRIMARY_OUTLET} from '../shared';
import {ComponentInputBindingOptions} from '../router_config';
import {BLOCKING_SYMBOL, InternalRouterResource} from '../router_resource';
...
export class RoutedComponentInputBinder {
  ...
  private subscribeToRouteData(outlet: RouterOutlet, injector: Injector) {
    ...
    if (this.feature?.createResourceOutletBindingEffects && outlet.activatedComponentRef) {
      const result = this.feature.createResourceOutletBindingEffects(
        outlet.activatedComponentRef,
        activatedRoute,
        injector,
      );
      effects.push(...result.effects);
      keysBoundToBlockingResources = result.handledKeys;
    }
