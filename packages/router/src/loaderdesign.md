...
Implement a modern data loading strategy for the Angular Router with the following features:

1. Parallel execution
2. Provides option for non-blocking
3. Reloadable
4. Reactive
5. Supports multiple results/streaming e.g. through reactivity or rxResource-like results
6. (Potentially) An option for eager parallel loaders that execute after matching but before guards.
...
interface Route {
  /**
   * A resources that runs EAGERLY, prior to guards and resolvers.
   * This resources will NOT have access to `resolvedData`.
   */
  eagerResources?: (ctx: EagerResourceContext) => ResourceResult;
...
Rationale: This provides an opt-in mechanism for initiating optimistic data fetches earlier in the navigation lifecycle for non-critical or universally accessible data. By keeping eagerResources separate, it avoids the "partial context" problem that would arise if a single resources function had to conditionally access resolvedData. It provides a clear contract and guarantees type safety for developers.
