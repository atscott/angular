...
export abstract class BasePageTitleStrategy implements PageTitleStrategy {
   ...
   */
  collectPageTitles(route: RouterStateSnapshot): string[] {
    let pageTitles: string[] = [];
    let stack: ActivatedRouteSnapshot[] = [route.root];
    while (stack.length > 0) {
      const route = stack.pop()!;
      const pendingResolvedPageTitle = route._resolve.pageTitle && !route._resolvedData;
      if (route.data.pageTitle && !pendingResolvedPageTitle) {
