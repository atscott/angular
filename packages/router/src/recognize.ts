...
export class Recognizer {
  ...
  processChildren(config: Route[], segmentGroup: UrlSegmentGroup):
    ...
    for (const childOutlet of Object.keys(segmentGroup.children)) {
      ...
      const routesWithMatchingOutlet = config.filter(r => getOutlet(r) === childOutlet);
      // First, try to get children against routes with a matching outlet
      let outletChildren = this.processSegmentGroup(routesWithMatchingOutlet, child, childOutlet);
      // If that fails, try matching against all routes. This would include empty path matches to
      // different outlets.
      if (outletChildren === null) {
        outletChildren = this.processSegmentGroup(config, child, childOutlet);
      }
      // If that still fails, return `null` - we could not find any matching route config for this
      // outlet so the recognize should fail.
      if (outletChildren === null) {
        return null;
      }
