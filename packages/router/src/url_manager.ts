...
import {NavigationEnd, NavigationStart, OutletActivationStart, RoutesRecognized} from './events';
import {RestoredState, Router} from './router';
...
export class ComputedUrlManager {
  ...
  private get urlUpdateStrategy() {
    return this.router.urlUpdateStrategy;
  }
  private get urlHandlingStrategy() {
    return this.router.urlHandlingStrategy;
  }
  ...
  constructor(
    ...
    router.events.subscribe(e => {
      if (e instanceof NavigationEnd) {
        ...
        this.lastSuccessfulRawUrlTree = this.router.getCurrentNavigation()!.finalUrl!;
        this.lastSuccessfulRouterUrlTree = this.router.getCurrentNavigation()!.extractedUrl;
      }
      if (e instanceof NavigationStart) {
        ...
        // Router?
        let restoredState = e.restoredState;
        ...
        if (restoredState && restoredState.ɵrouterPageId) {
          ...
          const replaceUrl = !!router.getCurrentNavigation()?.extras?.replaceUrl;
          const skipLocationChange = !!router.getCurrentNavigation()?.extras?.skipLocationChange;
      ...
      if (e instanceof RoutesRecognized) {
        if (this.urlUpdateStrategy === 'eager') {
          const t = this.router.getCurrentNavigation()!;
          if (!t.extras?.skipLocationChange) {
            const rawUrl = this.urlHandlingStrategy.merge(t.finalUrl!, t.initialUrl);
            this.setBrowserUrl(rawUrl);
          }
        }
      }
  ...
  restoreHistory(t: {source: string}) {
    ...
    const browserUrlUpdateOccurred =
        (t.source === 'popstate' || this.urlUpdateStrategy === 'eager' ||
         this.currentUrlTree === this.router.getCurrentNavigation()?.finalUrl);
...
export class ReplaceUrlManager {
  ...
  constructor(
    ...
    router.events.subscribe(e => {
      if (e instanceof NavigationEnd) {
        this.lastSuccessfulId = e.id;
        this.lastSuccessfulRouterUrlTree = this.router.getCurrentNavigation()!.finalUrl!;
      }
      if (e instanceof NavigationStart) {
      }
      if (e instanceof RoutesRecognized) {
  ...
  private resetState(): void {
    ...
    this.rawUrlTree = this.urlHandlingStrategy.merge(
        this.currentUrlTree, this.router.getCurrentNavigation()!.initialUrl);
    this.location.replaceState(
