  ...
  describe(`${browserAPI}-based routing`, () => {
    ...
    describe('navigation', function () {
      ...
      it('should set history.state when navigation with browser back and forward', fakeAsync(
        inject([Router, Location], (router: Router, location: Location) => {
          ...
          // TODO: This is a bit awkward with navigation API because we can't get the state unless we reach into history
      ...
      it('should not error if state is not {[key: string]: any}', fakeAsync(
        inject([Router, Location], (router: Router, location: Location) => {
          // TODO this is kind of a bit awkward. we can't modify the state on the event
          // triggered from the UI (location.back). we could allow it and then do a refresh...
          ...
          expect(getState()).toEqual({navigationId: 3});
        }),
      ));
    ...
    describe('duplicate in-flight navigations', () => {
      ...
      it('should not cause URL thrashing', async () => {
        ...
        trackUrlChanges(urlChanges, browserAPI);
    ...
    // TODO: infinite loop?
    xit('can redirect from error handler', async () => {
      TestBed.configureTestingModule({
        providers: [
          provideRouter(
            [
    ...
    (['deferred', 'eager'] as const).forEach((urlUpdateStrategy) => {
      describe(`urlUpdateStrategy: ${urlUpdateStrategy}`, () => {
        ...
        it('can renavigate to throwing component', fakeAsync(() => {
          ...
          advance(fixture); // have to commit the rollback - Navigation API events to not happen synchronously
          expect(location.path()).toEqual('');
      ...
      describe('data', () => {
        ...
        it('should not override nested route static data when both are using resolvers', fakeAsync(
          inject([Router], (router: Router) => {
            ...
            const pInj = fixture.debugElement.query(
              By.directive(NestedComponentWithData),
            ).injector!;
            const cmp = pInj.get(NestedComponentWithData);
        ...
        describe('should support history and state', () => {
          ...
          afterEach(fakeAsync(
            inject([Router, Location], (router: Router, location: Location) => {
              ...
              expect(getState()).toEqual({foo: 'bar', navigationId: 3});
            }),
          ));
        });
      ...
      describe('redirects', () => {
        ...
        it('should not break the back button when trigger by location change', fakeAsync(() => {
          ...
          advance(fixture); // needed for navigation API to resolve above navigations
        ...
        describe('CanActivate', () => {
          ...
          describe('should redirect when guard returns UrlTree', () => {
            ...
            it('can redirect while changing state object', fakeAsync(() => {
              ...
              expect(location.path()).toEqual('/redirected');
              expect(getState()).toEqual(jasmine.objectContaining({test: 1}));
            }));
          });
              ...
              providers: [
                Logger,
                {
                  ...
                  useFactory: (logger: Logger) => () => (
                    logger.add('canActivateChild_parent'), true
                  ),
                  deps: [Logger],
                },
                {
...
function getState(): any {
  return (
    TestBed.inject(PlatformNavigation)?.currentEntry?.getState() ??
    TestBed.inject(Location).getState()
  );
}

function simulateLocationChange(url: string, browserAPI: 'history' | 'navigation') {
  const location = TestBed.inject(Location);
  location.go(url);
  if (browserAPI === 'history') {
    location.historyGo(0); // need this for the popstate event
  }
}

function trackUrlChanges(urlChanges: string[], browserAPI: 'history' | 'navigation') {
  const location = TestBed.inject(Location);
  if (browserAPI === 'history') {
    ...
    // popstate doesn't happen, nor does the history manager go through location APIs
    // so onUrlChange doesn't work
    const navigation = TestBed.inject(PlatformNavigation);
    navigation.addEventListener('currententrychange', () => {
      urlChanges.push(location.getPathFromUrl(new URL(navigation.currentEntry!.url!)));
    });
  }
}
