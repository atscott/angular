...
fdescribe('recognize', () => {
  it('should work', () => {
    const s = checkRecognize([{path: 'a', component: ComponentA}], 'a')!;
  ...
  it('should freeze params object', () => {
    const s: RouterStateSnapshot =
        checkRecognize([{path: 'a/:id', component: ComponentA}], 'a/10')!;
    checkActivatedRoute(s.root, '', {}, RootComponent);
  ...
  it('should support secondary routes', () => {
    const s: RouterStateSnapshot = checkRecognize(
        [
  ...
  describe('data', () => {
    it('should set static data', () => {
      const s = checkRecognize([{path: 'a', data: {one: 1}, component: ComponentA}], 'a')!;
    ...
    it('should set resolved data', () => {
      const s =
          checkRecognize([{path: 'a', resolve: {one: 'some-token'}, component: ComponentA}], 'a')!;
  ...
  describe('empty path', () => {
    describe('root', () => {
      it('should work', () => {
        const s = checkRecognize([{path: '', component: ComponentA}], '')!;
        checkActivatedRoute(s.root.firstChild!, '', {}, ComponentA);
      });

      it('should match when terminal', () => {
        const s = checkRecognize([{path: '', pathMatch: 'full', component: ComponentA}], '')!;
        checkActivatedRoute(s.root.firstChild!, '', {}, ComponentA);
      });
    ...
    describe('aux split is in the middle', () => {
      ...
      it('should match (non-terminal) when both primary and secondary and primary has a child',
         () => {
           ...
           const s = checkRecognize(config, 'parent/b')!;
           checkActivatedRoute(s.root, '', {}, RootComponent);
    ...
    describe('with outlets', () => {
      ...
      it('c-c-c-c-combo: should work with children outlets and adjacent', () => {
        const s = checkRecognize(
            [
              {
                path: '',
      ...
      it('should work for outlets adjacent to empty path', () => {
        const s = checkRecognize(
            [
              {path: '', component: ComponentA, children: [{path: '', component: ComponentC}]},
  ...
  describe('wildcards', () => {
    it('should support simple wildcards', () => {
      const s = checkRecognize([{path: '**', component: ComponentA}], 'a/b/c/d;a1=11')!;
  ...
  describe('custom path matchers', () => {
    ...
    it('should work with terminal route', () => {
      ...
      const s = checkRecognize([{matcher, component: ComponentA}] as any, '')!;
      const a = s.root.firstChild!;
  ...
  describe('fragment', () => {
    it('should support fragment', () => {
      const config = [{path: 'a', component: ComponentA}];
      const s = checkRecognize(config, 'a#f1')!;
...
function checkRecognize(
    ...
    relativeLinkResolution: 'legacy'|'corrected' = 'legacy'): RouterStateSnapshot|null {
  return new Recognizer(
             RootComponent, config, tree(url), url, paramsInheritanceStrategy,
