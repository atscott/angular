...
class Scheduler {
  ...
  private flushPeriodic(doTick?: (elapsed: number) => void): number {
    ...
    // Find the last task currently queued in the scheduler queue and tick
    // till that time.
    const tickTime = this.timeUntilLastTimer();
    this.tick(tickTime, doTick);
    return tickTime;
  }

  timeUntilLastTimer(): number {
    if (this._schedulerQueue.length === 0) {
      return 0;
    }
    const startTime = this._currentTickTime;
    const lastTask = this._schedulerQueue[this._schedulerQueue.length - 1];
    return lastTask.endTime - startTime;
  }
...
class FakeAsyncTestZoneSpec implements ZoneSpec {
  ...
  async flushAsync() {
    ...
    if (this.tickMode.mode === 'manual') {
      throw new Error('Cannot flushAsync in manual mode.');
    }
    this.flushMicrotasks();
    if (this._scheduler.getTimerCount() === 0) {
      return 0;
    }

    const tickTime = this._scheduler.timeUntilLastTimer();
    await new Promise<void>((r) => {
      originalTimeout(() => {
        // we need to allow any synchronous flush calls to
        // complete before we resolve this promise calling
        // flush may result in additional calls to flush,
        // and those inner ones must complete
        // synchronously before the outer one completes.
        //
        // Now the question is whether that problem should be fixed here or if
        // the _caller_ of flushAsync should adjust to do handle this case and
        // wait a microtask. It's really odd that this would return a tick time
        // but the actual time might be further than that because of inner flush
        // calls.
        // Zone.root.run(() => {
        //   queueMicrotask(() => {
        //     r();
  ...
  private advanceUntilModeChanges(doTick?: (elapsed: number) => void): void {
    ...
    Zone.root.run(async () => {
      ...
      // macrotasks before the current microtasks are flushed.
